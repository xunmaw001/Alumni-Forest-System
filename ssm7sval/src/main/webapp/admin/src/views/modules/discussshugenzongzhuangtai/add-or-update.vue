...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `discussshugenzongzhuangtai/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `discussshugenzongzhuangtai/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.discussshugenzongzhuangtaiCrossAddOrUpdateFlag = false;
                  this.parent.search();
