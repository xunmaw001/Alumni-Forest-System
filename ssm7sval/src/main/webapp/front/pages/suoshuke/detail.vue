<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"180rpx","fontSize":"28rpx","color":"#333","textAlign":"left"}'>科：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.ke}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','suoshuke');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','suoshuke');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('suoshuke', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusssuoshuke/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('suoshuke', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('suoshuke', this.detail);
				this.$utils.msg('审核成功');
